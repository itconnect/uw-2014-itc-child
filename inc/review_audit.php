...
function reviewedOnAudit() {
	...
	echo '</tbody></table>';
	echo '<script>$("#reviewed tfoot th").each(function(){var e=$(this).text();$(this).html(\'<input type="text" placeholder="Search \'+e+\'" />\')});var table=$("#reviewed").DataTable({"pageLength": 100});table.columns().every(function(){var e=this;$("input",this.footer()).on("keyup change",function(){e.search()!==this.value&&e.search(this.value).draw()})});</script>';
	return ob_get_clean();
}
